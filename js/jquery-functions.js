$("document").ready(function () {
  ...
  function skipToEnd(message) {
    const errorEnd = document.createElement("h5"); //----------------------------------END
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν δικαιούστε να εκδώσετε αντίγραφο εντύπου άδειας οδήγησης."
        : "We are sorry but you are not entitled to issue a copy of a driver's license";
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Πρέπει να έχετε την κατοικία σας στην περιοχή της οικείας Υπηρεσίας Μεταφορών και Επικοινωνιών της Περιφερειακής Ενότητας.") : skipToEnd("You must have your residence in the area of ​​the relevant Transport and Communications Service of the Regional Unit.");
      }  else {
        ...
        if (currentQuestion === 1 && selectedRadioButtonIndex === 1) {
          currentQuestion += 1;
        }
